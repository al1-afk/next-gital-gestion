#!/usr/bin/env bash
# ════════════════════════════════════════════════════════════════════
#  GestiQ — Appliquer la migration 034 (SOPs Designer / Graphiste) en PROD
#
#  Lance la migration 034 (catégorie SOP « Designer / Graphiste » + 6 SOPs)
#  sur la base de données de production via SSH + docker exec.
#
#  Idempotent : safe à re-runner (WHERE NOT EXISTS sur chaque INSERT).
#
#  Usage :
#    bash scripts/apply-prod-sop-designer.sh
#
...
SQL_FILE="supabase/migrations/034_seed_designer_sops.sql"
...
BKP_NAME="pre-sop-designer-$(date +%Y%m%d-%H%M%S).sql"
...
echo "▶ Application de la migration 034…"
cat "$SQL_FILE" | ssh "$SSH_TARGET" "docker exec -i '$PG_CONT' psql -U '$PG_USER' -d '$PG_DB' -v ON_ERROR_STOP=1"

echo ""
echo "▶ Vérification — comptage des SOPs Designer :"
ssh "$SSH_TARGET" "docker exec '$PG_CONT' psql -U '$PG_USER' -d '$PG_DB' -c \"SELECT COUNT(*) AS total_sops, COUNT(DISTINCT tenant_id) AS tenants FROM sops WHERE category='designer';\""
...
echo "  2. Cliquer sur « Designer / Graphiste » dans la sidebar — 6 SOPs visibles"
echo ""
