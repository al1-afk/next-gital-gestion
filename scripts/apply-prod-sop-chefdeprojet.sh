#!/usr/bin/env bash
# ════════════════════════════════════════════════════════════════════
#  GestiQ — Appliquer la migration 030 (SOPs Chef de projet) en PROD
#
#  Lance la migration 030 (catégorie SOP « Chef de projet » + 5 SOPs)
#  sur la base de données de production via SSH + docker exec.
#
#  Idempotent : safe à re-runner (WHERE NOT EXISTS sur chaque INSERT).
#
#  Usage :
#    bash scripts/apply-prod-sop-chefdeprojet.sh
#
...
SQL_FILE="supabase/migrations/030_seed_chef_de_projet_sops.sql"
...
BKP_NAME="pre-sop-chefdeprojet-$(date +%Y%m%d-%H%M%S).sql"
...
echo "▶ Application de la migration 030…"
cat "$SQL_FILE" | ssh "$SSH_TARGET" "docker exec -i '$PG_CONT' psql -U '$PG_USER' -d '$PG_DB' -v ON_ERROR_STOP=1"

echo ""
echo "▶ Vérification — comptage des SOPs Chef de projet :"
ssh "$SSH_TARGET" "docker exec '$PG_CONT' psql -U '$PG_USER' -d '$PG_DB' -c \"SELECT COUNT(*) AS total_sops, COUNT(DISTINCT tenant_id) AS tenants FROM sops WHERE category='projets';\""
...
echo "  2. Cliquer sur « Chef de projet » dans la sidebar — 5 SOPs visibles"
echo ""
